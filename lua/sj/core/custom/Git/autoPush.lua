...
local function generate_git_summary(git_root)
	...
	local parts = {}
	if #added > 0 then
		table.insert(parts, "🆕 Added: " .. table.concat(added, ", "))
	end
	if #modified > 0 then
		table.insert(parts, "✏️ Updated: " .. table.concat(modified, ", "))
	end
	if #deleted > 0 then
		table.insert(parts, "🗑️ Removed: " .. table.concat(deleted, ", "))
	end
		...
		on_exit = function()
			...
			vim.fn.jobstart(cmd, {
				on_exit = function(_, code)
					vim.schedule(function()
						if code == 0 then
							vim.notify("✅ Auto pushed: " .. git_root, vim.log.levels.INFO)
						else
							vim.notify("❌ Git push failed: " .. git_root, vim.log.levels.ERROR)
						end
					end)
				end,
			})
