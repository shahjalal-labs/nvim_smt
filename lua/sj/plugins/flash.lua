return {
  ...
  function()
    require("flash").jump({
      pattern = [[\k\+]], -- match Vim 'word' characters
      search = {
        mode = "search",
        multi_window = false, -- set true if you want to search all windows
        incremental = false,
      },
      label = {
        after = true,
      ...
      action = function(match)
        -- Get word text directly from the line using match positions
        ...
        -- Yank to default and system clipboard registers
        if word and #word > 0 then
          vim.fn.setreg('"', word)
          vim.fn.setreg('+', word)
          vim.notify('⚡ Yanked word: "' .. word .. '"', vim.log.levels.INFO)
        else
  ...
  desc = "Flash hint all visible words and yank chosen one",
}
